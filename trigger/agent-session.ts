...
export const agentSession = task({
  ...
  run: async (payload: AgentSessionPayload) => {
    ...
    const markers = stdout
      .split("\n")
      .filter((l) => /^(GATE_RESULT|PUSHED|COMMIT_SHA|COMMIT_FAILED|SESSION_\w+|NO_CHANGES|DENYLIST_HIT|CLONE_FAILED|NPM_CI_FAILED|REBASE_FAILED|PUSH_FAILED|DRY_RUN|PHASE)=?/.test(l))
      .join("\n");
