...
describe("prepareAgentEmail (autonomous-send gate)", () => {
  const self = "demo@agents.looplabs.fun";
  it("accepts a well-formed email, lowercasing the recipient", () => {
    expect(
      prepareAgentEmail({ to: "Founder@Startup.IO", subject: "Hi", body: "Intro" }, self)
    ).toEqual({ to: "founder@startup.io", subject: "Hi", text: "Intro" });
  });
  it("rejects a malformed recipient, the agent's own address, or empty fields", () => {
    expect(prepareAgentEmail({ to: "not-an-email", subject: "s", body: "b" }, self)).toBeNull();
    expect(prepareAgentEmail({ to: "a@b.com, c@d.com", subject: "s", body: "b" }, self)).toBeNull();
    expect(prepareAgentEmail({ to: self, subject: "s", body: "b" }, self)).toBeNull(); // no self-loop
    expect(prepareAgentEmail({ to: "a@b.com", subject: "", body: "b" }, self)).toBeNull();
    expect(prepareAgentEmail({ to: "a@b.com", subject: "s", body: "" }, self)).toBeNull();
    expect(prepareAgentEmail(null, self)).toBeNull();
  });
  it("clamps subject (200) and body (4000)", () => {
    const out = prepareAgentEmail(
      { to: "a@b.com", subject: "x".repeat(300), body: "y".repeat(5000) },
      self
    );
    expect(out?.subject.length).toBe(200);
