...
export interface FeedItem {
  ...
  /** Epoch ms for chronological merge with the chat feed (0 when unknown). */
  ts?: number;
  text: string;
  /** escalation/proposal lifecycle */
  status?: "open" | "applied" | "adopted" | "declined";
  /**
   * Founder execution-triage for an ADOPTED proposal: `done` (already shipped),
   * `todo` (queued for the agent to build next — the agent's work queue), or
   * `refused` (founder overrides the passed vote). Undefined until triaged.
   */
  exec?: "todo" | "done" | "refused";
