...
/**
 * Build the `agent_emails` "out" row for an email the agent just sent. Mirrors
 * inboundRow but the conversation `party` is the recipient, so the Agent Console
 * shows the agent's own outbound messages alongside the replies.
 */
export function outboundRow(
  projectKey: string,
  p: { to: string; subject: string; text: string }
): InboundEmailInsert {
  return {
    project_key: projectKey,
    direction: "out",
