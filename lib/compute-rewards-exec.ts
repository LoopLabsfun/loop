 ...
 * device's ledger, and stamps rewarded_at so a re-run can never double-credit
 * the same contribution. Disarmed by default: a rate of 0 (unset
 * COMPUTE_REWARD_LAMPORTS_PER_UNIT) means this is a costless no-op — the
 * founder sets the rate when ready to actually start paying.
 *
 * Real money doesn't move here — this only credits an internal ledger. The
 * SOL send is lib/compute-rewards-payout.ts, a separate, more tightly gated
 * step.
 */

export function computeRewardRateLamports(env: Record<string, string | undefined> = process.env): number {
  const n = Number(env.COMPUTE_REWARD_LAMPORTS_PER_UNIT);
...
export async function accrueComputeRewards(
  ...
  const rate = computeRewardRateLamports(env);
  if (rate <= 0) {
    return { ok: true, devices: 0, totalLamports: 0, note: "disarmed (set COMPUTE_REWARD_LAMPORTS_PER_UNIT)" };
  }
  if (!supabaseAdmin) return { ok: false, devices: 0, totalLamports: 0, note: "no service-role client" };
  ...
  if (!assists.length && !checks.length) {
    return { ok: true, devices: 0, totalLamports: 0, note: "nothing new to accrue" };
  }
  ...
  for (const p of plan) {
    const { data: existing } = await supabaseAdmin
      ...
      .select("earned_lamports")
      .eq("device_id", p.deviceId)
      .maybeSingle();
    const prevEarned = Number((existing as { earned_lamports?: number } | null)?.earned_lamports ?? 0);
    await supabaseAdmin.from("compute_rewards").upsert(
      {
        device_id: p.deviceId,
        payout_address: p.payoutAddress,
        payout_address_hood: p.payoutAddressHood,
        earned_lamports: prevEarned + p.addLamports,
  ...
  // credited again next pass; the earned_lamports upsert above is what would
  // need to be idempotency-guarded if that mattered, but re-crediting an
  ...
  return {
    ok: true,
    devices: plan.length,
    totalLamports: plan.reduce((s, p) => s + p.addLamports, 0),
