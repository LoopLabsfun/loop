...
describe("densify", () => {
  ...
  it("fills gaps between sparse candles with flat candles at previous close", () => {
    ...
    const rows = [
      [t0, 1, 2, 0.5, 1.5],
      [t0 + 3 * HOUR, 1.6, 1.8, 1.4, 1.7], // 2-bucket gap
    ];
    const out = densify(rows, HOUR, 60);
    // t0, +1h flat, +2h flat, +3h real, then flat to now (2 more)
    expect(out.length).toBe(6);
    expect(out[1]).toEqual({ o: 1.5, h: 1.5, l: 1.5, c: 1.5 });
    expect(out[2]).toEqual({ o: 1.5, h: 1.5, l: 1.5, c: 1.5 });
    expect(out[3]).toEqual({ o: 1.6, h: 1.8, l: 1.4, c: 1.7 });
    expect(out[5]).toEqual({ o: 1.7, h: 1.7, l: 1.7, c: 1.7 });
  });
  ...
  it("caps output at limit, keeping the most recent buckets", () => {
    ...
    expect(out[59]).toEqual({ o: 1, h: 1, l: 1, c: 1 });
  });
