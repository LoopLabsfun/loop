...
import { commitHashForTitle, shareOnXUrl, type MatchCommit } from "@/lib/live-log";
  ...
  return (
    <div className="text-[12.5px] flex items-baseline gap-2 w-full animate-fadeInFast">
      ...
      <span className="ml-auto flex items-baseline gap-2 whitespace-nowrap font-mono text-[10.5px]">
        ...
        {t.status === "shipped" && (
          <a
            href={shareOnXUrl(t.title)}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            title="Share on X"
            className="text-muted hover:text-canvas transition-colors inline-flex items-center"
          >
