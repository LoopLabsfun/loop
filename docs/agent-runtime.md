...
> autonomous agents — one per project — modelled on the autonomous-operator pattern
...
- **Do not auto-create Twitter/X accounts.** It violates X's ToS, is bot-flagged
  ...
  common false expectation — the reference operator's headline handle is a *human-created* account the
...
- **Reddit** works for outreach but is rate-limited and community-moderated —
  keep it mandate-gated (drafts escalate before posting), exactly as prior operators
